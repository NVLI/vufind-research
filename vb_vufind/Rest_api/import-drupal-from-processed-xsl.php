...
foreach ($oai_pmh_list as $oai_pmh) {
  $oai_pmh_dir = "$vufind_local_dir/harvest/$oai_pmh/processed";
  $oai_pmh_log_dir = "$vufind_local_dir/harvest/$oai_pmh";
  $harvested_files = glob("$oai_pmh_dir/*.xml");
$harvested_files = array('/usr/local/vufind/local/harvest/museum/processed/1470304668_cdac_mu_1804.xml');
  // Get harvest xml id for which entity is been created till now.
  if (!file_exists($oai_pmh_log_dir . '/drupal-harvest-export.log')) {
    ...
    $file = fopen($oai_pmh_log_dir . '/drupal-harvest-export.log', 'a+');
    $processed_ids = array();
    ...
    $file = fopen($oai_pmh_log_dir . '/drupal-harvest-export.log', 'a+');
    $processed_ids = file_get_contents($oai_pmh_log_dir . '/drupal-harvest-export.log');
