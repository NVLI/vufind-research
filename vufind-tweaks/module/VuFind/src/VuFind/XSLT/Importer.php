...
class Importer implements ServiceLocatorAwareInterface
{
     ...
     * @param bool $testMode
     */
    public function saveToDrupal($xmlFile, $properties, $index = 'Solr',
        $testMode = false
    ) {
       $xml = $this->gernerateDrupalXML($xmlFile, $properties);
        // Save the results (or just display them, if in test mode):
        if (!$testMode) {
            $this->createEntityFromXML($xml);
        } else {
            Console::write($xml . "\n");
        }
    }
     ...
     * @param $xml
     */
    protected function createEntityFromXML($xml, $properties) {
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $options['General']['drupal_base_url'] . "/rest/session/token");
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
        $token= curl_exec($curl);
        $curl = curl_init($options['General']['drupal_base_url'] . '/entity/node');
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_USERPWD, "admin:admin");
        curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/xml', 'X-CSRF-Token: ' . $token));
        curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query('<doc>' . $data . '</doc>'));
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
     ...
     * @param $xml
     */
    protected function getNodeTypeFromXML($xml) {
